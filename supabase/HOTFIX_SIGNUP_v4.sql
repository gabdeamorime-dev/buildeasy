-- BuildEasy HOTFIX v4 — inscription (schéma schema.sql : name, ch_ids bigint[], org_id NOT NULL)
...
-- chantier_ids : ne pas forcer integer[] si la colonne existe déjà en bigint[]
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    ALTER TABLE public.profiles ADD COLUMN chantier_ids bigint[] NOT NULL DEFAULT '{}';
  END IF;
END $$;
...
DO $$
BEGIN
  IF to_regprocedure('public.be_profile_org_id()') IS NOT NULL THEN
    EXECUTE $p$
      CREATE POLICY billing_select ON public.billing_subscriptions
      FOR SELECT TO authenticated
      USING (org_id = public.be_profile_org_id())
    $p$;
  ELSIF to_regprocedure('public.user_org_id()') IS NOT NULL THEN
    EXECUTE $p$
      CREATE POLICY billing_select ON public.billing_subscriptions
      FOR SELECT TO authenticated
...
-- ── 5) Trigger inscription (profil seul, org via finish_signup) ───
...
DECLARE
  ...
  END;
  has_chantier_ids BOOLEAN;
  has_ch_ids BOOLEAN;
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'profiles' AND column_name = 'chantier_ids'
  ) INTO has_chantier_ids;
  ...
  IF has_chantier_ids THEN
    INSERT INTO public.profiles (id, nom, role, chantier_ids, email, vierge)
    VALUES (
      NEW.id, user_nom, user_role, '{}'::bigint[], COALESCE(NEW.email, ''),
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      updated_at = now();
  ELSIF has_ch_ids THEN
    INSERT INTO public.profiles (id, nom, role, ch_ids, email, vierge)
    VALUES (
      NEW.id, user_nom, user_role, '{}'::bigint[], COALESCE(NEW.email, ''),
      CASE WHEN is_signup THEN true ELSE COALESCE((NEW.raw_user_meta_data->>'vierge')::boolean, false) END
    )
    ON CONFLICT (id) DO UPDATE SET
      ...
      updated_at = now();
  ELSE
    INSERT INTO public.profiles (id, nom, role, email, vierge)
    VALUES (
      NEW.id, user_nom, user_role, COALESCE(NEW.email, ''),
      CASE WHEN is_signup THEN true ELSE COALESCE((NEW.raw_user_meta_data->>'vierge')::boolean, false) END
    )
    ON CONFLICT (id) DO UPDATE SET
      email = COALESCE(NULLIF(EXCLUDED.email, ''), public.profiles.email),
